      ...
      <div class="card">
        ...
        <div class="card-footer text-right" style="background:#C5C6C7">
          <span style="font-size: 12px">
            <strong>Dibuat Pada: </strong>{{  $category->created_at->dayName." | ".$category->created_at->day." ".$category->created_at->monthName." ".$category->created_at->year}} | {{$category->created_at->format('h:i:s')}} WIB | <a href="{{route('karyawan.detail',$category->createdBy->employee->id)}}" class="text-info">{{$category->createdBy->employee->nama}}</a> / <strong>Diubah Pada: </strong>{{  $category->updated_at->dayName." | ".$category->updated_at->day." ".$category->updated_at->monthName." ".$category->updated_at->year}} | {{$category->updated_at->format('h:i:s')}} WIB | {{$category->updated_at->format('h:i:s A')}} | <a href="{{route('karyawan.detail',$category->updatedBy->employee->id)}}" class="text-info">{{$category->updatedBy->employee->nama}}</a>
        </span>
